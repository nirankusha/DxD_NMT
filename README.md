...
| Model*          | sample\_k     |         sample |                 `K` |                `K` | `runs.parquet` (`N×K` rows)                                         |
...
| Plain sample (top-K)   | `sample_k`     | `sample_k`                     | All K → `runs.parquet`                                              |
...
| `runs.parquet`             | Long table: all beam-`K`sample-`K` and external-MBR sampled candidates |
